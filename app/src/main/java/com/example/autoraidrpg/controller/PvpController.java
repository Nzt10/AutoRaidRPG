...
public class PvpController extends Controller {
    ...
    private List<PreparationUnitInfoAdapter> allyUnits, enemyUnits;
    ...
    public PvpController(IView view) {
        super(view);
    }

    @Override
    public void init() {
        activity = (PvpActivity) view;
        executor = Executors.newFixedThreadPool(LoadImageTask.NUMBER_OF_THREADS);

        Intent intent = activity.getIntent();
        StageObserver.getInstance().setState(StageState.PVP);
        ...
        enemyUnits = (List<PreparationUnitInfoAdapter>) intent.getSerializableExtra("enemyUnits");
    ...
    private void setBattleFormation() {
        ...
        preparation(allyUnits, allies, frontlinerLeftViews, backlinerLeftViews);
        preparation(enemyUnits, hostiles, frontlinerRightViews, backlinerRightViews);
    ...
    private void preparation(List<PreparationUnitInfoAdapter> preparedUnits, BattleFormation battleFormation, List<View> frontlinerViews, List<View> backlinerViews) {
        ...
        for(int i = 0; i < preparedUnits.size(); i++) {
            ...
            if(unitInfo.getPosition().isFront()) battleFormation.addToFront(unitInfo.getPosition().getIndex(), unit, getBasedBattleUnitConsumer(frontlinerViews), getBattleUnitConsumer(frontlinerViews));
            else battleFormation.addToBack(unitInfo.getPosition().getIndex(), unit, getBasedBattleUnitConsumer(backlinerViews), getBattleUnitConsumer(backlinerViews));
        }
    }
